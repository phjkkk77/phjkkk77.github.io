...
function renderHeadToHead(summary) {
  // 실제 데이터 구조: team_head_to_head에 hanwha_vs_lotte_record 포함
  const headToHeadData = summary?.team_head_to_head;
  const hanwhaData = summary?.hanwha_vs_lotte;
  const lotteData = summary?.lotte_vs_hanwha;

  if (!headToHeadData || !headToHeadData.hanwha_vs_lotte_record) {
    console.warn("[DATA MISSING] team_head_to_head.hanwha_vs_lotte_record 필드가 없습니다.");
    nodes.headLeft.innerHTML = cardError("상대전적 데이터를 불러올 수 없습니다.");
    nodes.headRight.innerHTML = cardError("데이터 파일을 확인해주세요.");
    return;
  }

  const recordParsed = parseRecord(headToHeadData.hanwha_vs_lotte_record);
  if (!recordParsed) {
    console.warn(`[DATA ERROR] 상대전적 형식 오류: ${headToHeadData.hanwha_vs_lotte_record}`);
    nodes.headLeft.innerHTML = cardError("상대전적 데이터 형식이 잘못되었습니다.");
    nodes.headRight.innerHTML = cardError("데이터를 재구성해주세요.");
    return;
  }

  const { w, l, d, total } = recordParsed;
  const recordText = `${total}전 ${w}승 ${l}패`;
  const recordTextWithDraws = d > 0 ? `${recordText} ${d}무` : recordText;

  // 한화 기준 카드
  nodes.headLeft.innerHTML = `
    <span class="badge">상대전적 세부 (한화 기준)</span>
    <h3>한화 이글스 vs 롯데 자이언츠</h3>
    <div class="stat-grid">
      <div class="stat-item"><div class="label">맞대결</div><div class="value">${recordTextWithDraws}</div></div>
      <div class="stat-item"><div class="label">전적</div><div class="value">${w}승 ${l}패</div></div>
      <div class="stat-item"><div class="label">무승부</div><div class="value">${d}</div></div>
      <div class="stat-item"><div class="label">우세</div><div class="value">${w > l ? "한화 우위" : w < l ? "뒤처음" : "동등"}</div></div>
    </div>
    ${hanwhaData ? `<p class="section-sub">롯데 상대 한화 TOP 5 타자와 선발투수 기록이 상단 요약에 포함되었습니다.</p>` : ""}
  `;

  // 롯데 기준 카드 (반대 기록)
  const lotteRecordText = `${total}전 ${l}승 ${w}패`;
  const lotteRecordTextWithDraws = d > 0 ? `${lotteRecordText} ${d}무` : lotteRecordText;

  nodes.headRight.innerHTML = `
    <span class="badge">상대전적 세부 (롯데 기준)</span>
    <h3>롯데 자이언츠 vs 한화 이글스</h3>
    <div class="stat-grid">
      <div class="stat-item"><div class="label">맞대결</div><div class="value">${lotteRecordTextWithDraws}</div></div>
      <div class="stat-item"><div class="label">전적</div><div class="value">${l}승 ${w}패</div></div>
      <div class="stat-item"><div class="label">무승부</div><div class="value">${d}</div></div>
      <div class="stat-item"><div class="label">우세</div><div class="value">${l > w ? "롯데 우위" : l < w ? "뒤처침" : "동등"}</div></div>
    </div>
    ${lotteData ? `<p class="section-sub">한화 상대 롯데 TOP 5 타자와 선발투수 기록이 상단 요약에 포함되었습니다.</p>` : ""}
  `;
}
...
function renderTableById(tableId) {
  ...
  tableState.mountNode.innerHTML = `
    ...
    <p class="section-sub">컬럼 헤더 클릭으로 정렬 · 정렬 후 상위 5명부터 다시 표시됩니다.</p>
...
function warnMissingFields(datasetName, rows, requiredFields) {
  if (!rows.length) {
    console.warn(`[DATA QUALITY] ${datasetName}: 데이터 행이 없습니다. (0개)`);
    return requiredFields;
  }

  // 변경된 로직: 첫 행만이 아니라 전체 행 검사
  const missingByField = {};
  
  requiredFields.forEach((field) => {
    const missingCount = rows.filter((row) => !row[field] || String(row[field]).trim() === "").length;
    missingByField[field] = {
      total: rows.length,
      missing: missingCount,
      complete: rows.length - missingCount,
      percentage: Math.round(((rows.length - missingCount) / rows.length) * 100),
    };
  });

  // 누락된 필드 (100% 빠져 있는 필드)
  const completelyMissing = requiredFields.filter((field) => missingByField[field].missing === rows.length);
  
  // 부분 누락된 필드 (일부만 빠져 있는 필드)
  const partiallyMissing = requiredFields.filter(
    (field) => missingByField[field].missing > 0 && missingByField[field].missing < rows.length
  );

  // 로그 출력
  if (completelyMissing.length > 0) {
    console.warn(
      `[DATA MISSING] ${datasetName}: 완전 누락 필드 -> ${completelyMissing.join(", ")} (모든 ${rows.length}개 행)`
    );
  }

  if (partiallyMissing.length > 0) {
    const partialInfo = partiallyMissing
      .map((field) => `${field} (${missingByField[field].complete}/${rows.length}, ${missingByField[field].percentage}%)`)
      .join(", ");
    console.warn(`[DATA INCOMPLETE] ${datasetName}: 부분 누락 필드 -> ${partialInfo}`);
  }

  if (completelyMissing.length === 0 && partiallyMissing.length === 0) {
    console.log(`[DATA QUALITY] ${datasetName}: 모든 필드 완전 (${rows.length}개 행, ${requiredFields.length}개 필드)`);
  }

  return completelyMissing;
}
